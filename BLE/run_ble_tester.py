...
async def write_target(target, attribute, bytes):
    ...
    try:
        ...
        print(color(
            f'[OK] WRITE Handle 0x{attribute.handle:04X} --> Bytes={len(bytes_to_write):02d}, Val={hexlify(bytes_to_write).decode()}', 'green'))
        return True
    except ProtocolError as error:
        print(
            color(f'[!]  Cannot write attribute 0x{attribute.handle:04X}:', 'yellow'), error)
    except TimeoutError:
...
async def read_target(target, attribute):
    ...
    try:
        ...
        print(color(
            f'[OK] READ  Handle 0x{attribute.handle:04X} <-- Bytes={len(read):02d}, Val={read.hex()}', 'cyan'))
        return value
    except ProtocolError as error:
        print(
            color(f'[!]  Cannot read attribute 0x{attribute.handle:04X}:', 'yellow'), error)
    except TimeoutError:
...
class TargetEventsListener(Device.Listener):
    ...
    async def on_connection(self, connection):
        ...
        inputNum = input_file  # ARRAY LOCATION HERE, argument
